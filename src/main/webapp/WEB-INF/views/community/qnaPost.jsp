			...
			<div class="grid_center_R">
				...
				<table id="posttable">
					<tr>
                        ...
                        <a href="javascript:void(0);" onclick="window.open('report.do?target_type=qna_post&post_no=${post.post_no}&creator_id=${post.creator_id}', 'reportPopup', 'width=500,height=600,resizable=no,scrollbars=no')">
				...
				<div id="comment_form">
					<form action="/fitralpark/qnaPostOK.do" method="post">
				...
				<div id="button_area">
					...
					<div class="right_button">
						<button type="button" onclick="location.href='qnaPostEdit.do?post_no=${post.post_no}'">수정</button>
						<button type="button" onclick="window.open('qnaPostDel.do?post_no=${post.post_no}', 'deletePopup', 'width=500,height=300,resizable=no,scrollbars=no')">삭제</button>
					</div>
				</div>
				</div>
				</div>
...
<script>
	...
	$(document).ready(function() {
		...
		function updateRecommend(post_no, vote_check) {
			$.ajax({
				url: "${pageContext.request.contextPath}/qnaPostOK.do",
				type: "POST",
				data: {
					post_no: post_no,
	...
	function saveComment(commentNo) {
		...
		$.ajax({
			url: '/fitralpark/qnaCommentEditOK.do',
			type: 'POST',
			data: {
	...
	function deleteComment(commentNo, creatorId) {
		if(confirm('댓글을 삭제하시겠습니까?')) {
			...
			form.method = 'POST';
			form.action = 'qnaCommentDelOK.do';
