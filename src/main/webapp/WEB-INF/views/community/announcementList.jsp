				...
				<div id="mainbox">
					<div id="smallbox">
						...
						<table id="boardtable" >
							...
							<c:forEach items="${announcement_list}" var="dto" varStatus="status">
							<tr>
								<td>${(page - 1) * pageSize + status.index + 1}</td>
								<td><a href="announcementPost.do?post_no=${dto.post_no}
						...
						<div class="pagination">
							<c:if test="${page > 1}">
								<a href="announcementList.do?page=${page-1}&search_category=${searchCategory}&search_sel=${searchSel}&searchWord=${searchWord}">이전</a>
							</c:if>
							...
							<c:forEach begin="${startPage}" end="${endPage}" var="i">
								<c:choose>
									...
									<c:otherwise>
										<a href="announcementList.do?page=${i}&search_category=${searchCategory}&search_sel=${searchSel}&searchWord=${searchWord}">${i}</a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							
							<c:if test="${page < totalPages}">
								<a href="announcementList.do?page=${page+1}&search_category=${searchCategory}&search_sel=${searchSel}&searchWord=${searchWord}">다음</a>
							</c:if>
						</div>
					...
					<form action="/fitralpark/announcementList.do" method="get">
					<div id="board_etc_box">
						...
						<button type="button" id="btnadd_post" onclick="location.href='announcementPostWrite.do'">글쓰기</button>
					</div>
					</form>
				</div>
...
<script>
    ...
    location.href = '/fitralpark/announcementList.do?search_category=' + category + '&search_sel=' + sel + '&searchWord=' + word;
}
